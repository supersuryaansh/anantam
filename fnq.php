<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="./css/about.css" />
    <link rel="stylesheet" href="./css/fnq.css"/>
    <link rel="tab icon" href="./assets/images/icon/ANANTAMLOGO.svg" />
    <title>ANANTAM - REGISTRATION</title>
</head>
<body>
<img class="bg--img" src="./assets/images/fnqBg.jpg"/>
<div class="main--menuPage">
    <button class="menu--close-btn">
        <img src="../assets/images/icon/closeBtn.svg" alt="" />
    </button>
    <div class="menuPage--content">
        <img src="../assets/images/menuPageLine/lt-corner.svg" alt="" />
        <a href="./sponsors.php">SPONSORS</a>
        <img src="../assets/images/menuPageLine/rt-corner.svg" alt="" />
        <a href="./developer.php">DEVELOPERS</a>
        <img
                class="logo--img"
                src="../assets/images/icon/BLACK LOGO.svg"
                alt="" />
        <a href="./prizepool.php">PRIZE POOL</a>
        <img src="../assets/images/menuPageLine/lb-corner.svg" alt="" />
        <a href="./gallery.php">GALLERY</a>
        <img src="../assets/images/menuPageLine/rb-corner.svg" alt="" />
    </div>
</div>
<nav class="desktop--nav">
    <img class="line1" src="./assets/images/webLINES/navLine.png" alt="" />
    <span>
        <a href="index.php">
            <img src="./assets/images/icon/LOGO-TYPOGRAPGY.png" alt="" />
            <p class="date--of-event">08 JAN, 2024 - 11 JAN, 2024</p>
        </a>
    </span>
    <span class="nav--links">
    <a>ABOUT</a>
    <a>EVENTS</a>
    <a>SYMPOSIUM</a>
    <a>REGISTER</a>
    <button class="nav--btn">
        <img src="./assets/images/icon/menu.svg" alt="" />
    </button>
        </span>
</nav>
<main>
    <h1 class="event--heading">F & Q :</h1>
    <div class="question--area">
        <span>
            <h1 class="question">
                1. What is ANANTAM?
            </h1>
            <p class="answer">
                   ANANTAM is a collaborative and grand-scale technical fest organized by IEEE BKBIET Student Branch and GDSC BKBIET Techfest. It features a diverse range of events, including hackathons, robotics competitions, DSA competitions, quizzes, speaker sessions, and various fun activities.
            </p>
        </span>
        <span>
            <h1 class="question">
                2. When and Where will ANANTAM take place?
            </h1>
            <p class="answer">
                   Provide details about the date, time, and venue of ANANTAM.
            </p>
        </span>
        <span>
            <h1 class="question">
                3. How can I register for ANANTAM events?
            </h1>
            <p class="answer">
           Include information on the registration process, deadlines, and any associated fees.
            </p>
        </span>
        <span>
            <h1 class="question">
               4. Are there any prerequisites for participating in specific events?
            </h1>
            <p class="answer">
                   Mention if certain events have specific requirements or eligibility criteria.
            </p>
        </span>
        <span>
            <h1 class="question">
                5. Can I participate in multiple events?
            </h1>
            <p class="answer">
                     Clarify if participants can register for and participate in multiple events during the fest.
            </p>
        </span>
        <span>
            <h1 class="question">
           6. Is there a registration fee for ANANTAM?
            </h1>
            <p class="answer">
                     Specify if there are any costs associated with participating in ANANTAM and how payment can be made.
            </p>
        </span>
        <span>
            <h1 class="question">
                7. What is the format of the hackathon?
            </h1>
            <p class="answer">
                   Provide details about the structure, rules, and duration of the hackathon.
            </p>
        </span>
        <span>
            <h1 class="question">
                8. How are winners determined for the competitions?
            </h1>
            <p class="answer">
                  Explain the criteria and judging process for different events.
            </p>
        </span>
        <span>
            <h1 class="question">
               9. Are there any accommodation facilities for out-of-town participants?
            </h1>
            <p class="answer">
                     If applicable, provide information about lodging options and any associated costs.
            </p>
        </span>
        <span>
            <h1 class="question">
                10. Can I attend speaker sessions without participating in the competitions?
            </h1>
            <p class="answer">
                       Clarify if there are separate registrations for attending speaker sessions or if it is included in the general registration.
            </p>
        </span>
        <span>
            <h1 class="question">
                11. How can I get updates and announcements about ANANTAM?
            </h1>
            <p class="answer">
                       Provide information about official communication channels, such as social media, websites, or newsletters.
            </p>
        </span>
        <span>
            <h1 class="question">
               12. Will certificates be provided for participation?
            </h1>
            <p class="answer">
                           Clarify whether participants will receive certificates and how they can be obtained.
            </p>
        </span>

    </div>
</main>
<img
        class="bottom--line"
        src="./assets/images/webLINES/navLine.png"
        alt="" />


<footer>
    <div class="maker--logo">
        <span>
          <a href=""><img src="./assets/images/icon/IEEE.png" alt="" /></a>
          <a
                  href="https://gdsc.community.dev/b-k-birla-institute-of-engineering-technology-pilani/"
          ><img class="gdsc--logo" src="./assets/images/icon/GDSC.svg" alt=""
              /></a>
          <a href="https://bkbiet.ac.in/"
          ><img class="bkbiet--logo" src="./assets/images/icon/BKBIET.png" alt=""
              /></a>
        </span>
    </div>
    <div class="footer--content">
        <span class="links">
          <h1>LINKS</h1>
          <a href="/events.php">events</a>
          <a href="/register.php">registration</a>
          <a href="/login.php">sign in</a>
          <a href="">developers</a>
          <a href="">prize pool</a>
          <a href="">sponsors</a>
          <a href="fnq.php">F&Q</a>
        </span>
        <span class="contacts">
          <h1>CONTACT US</h1>
          <div>
            <span>
              <h2>Mr. Sachin</h2>
              <p>Chairperson, IEEE BKBIET SB</p>
              <a href="tel:+91-9772541952"
              ><img
                          src="/assets/images/icon/PHONE.png"
                          alt="" />+91-9772541952</a
              >
            </span>
            <span>
              <h2>Mr. Aditya Sharma</h2>
              <p>Lead, GDSC BKBIET</p>
              <a href="tel:+91-8250639553 "
              ><img
                          src="/assets/images/icon/PHONE.png"
                          alt="" />+91-8250639553</a
              >
            </span>
          </div>
          <a href="mailto:anantam@bkbiet.ac.in" class="mail" target="_blank"
          ><img
                      src="./assets/images/icon/gmail.png"
                      alt="" />anantam@bkbiet.ac.in</a
          >
        </span>
        <span class="address">
          <h1>ADDRESS</h1>
          <a  href="https://maps.app.goo.gl/TUQJHh7gtAHLH32W7"
          >B K Birla Institute of Engineering and Technology, Ceeri Road,
            Jhunjhunu, Pilani, Rajasthan 333031</a
          >
          <a href="https://bkbiet.ac.in/" target="_blank" class="mail"
          >https://bkbiet.ac.in/</a
          >
        </span>
    </div>
    <div class="social--links">
        <a href="mailto:anantam@bkbiet.ac.in"><img src="./assets/images/icon/gmail.png" alt="" /></a>
        <a href="https://www.instagram.com/bkbiet.anantam/"><img src="./assets/images/icon/instagram.png" alt="" /></a>
    </div>
    <p class="copyright">&COPY; made by ANANTAM TEAM, with 💓💓💓</p>
</footer>
<script src="./js/registration.js"></script>
</body>
</html>
